...
public class CamminoHamiltonianoPerEsiste {

  public static boolean risposta(boolean[][] grafo, int[] soluzione, int j) {
    if (!completa(grafo, soluzione, j)) {
      if (!rifiuta(grafo, soluzione, j)) {
        int i = j;
        boolean accetta = false;
        while (i < soluzione.length && !accetta) {
          swap(soluzione, i, j);
          accetta = risposta(grafo, soluzione, j + 1);
          swap(soluzione, i, j);
          i++;
        }
        return accetta;
  ...
  private static boolean completa(boolean[][] grafo, int[] soluzione, int j) {
