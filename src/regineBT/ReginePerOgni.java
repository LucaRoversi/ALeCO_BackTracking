...
/** Stampa tutte configurazioni di n regine piazzate
...
public class ReginePerOgni {
  
  public static int tentativi = 0;

  public static void soluzioni(int[] regine, int j) {

    if (!completa(regine, j)) {
      if (!rifiuta(regine, j)) {
        int i = j;
        while (i < regine.length) {
          swap(regine, i, j);
          soluzioni(regine, j + 1);
          swap(regine, i, j);
          i++;
        }
      } else {
