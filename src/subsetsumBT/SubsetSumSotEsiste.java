...
public class SubsetSumSotEsiste {
  ...
  public static boolean risposta(int[] insieme, int sommaCercata, 
    ...
    if (!completa(soluzione, j)) {
      if (!rifiuta(insieme, sommaCercata, soluzione, j)) {
        soluzione[j] = true;
        boolean risposta = risposta(insieme, sommaCercata, soluzione, j + 1);
        if (!risposta) {
  ...
  private static boolean rifiuta(int[] insieme, int sommaCercata, 
    ...
    int estesioneSomma = 0;
    if (somma < sommaCercata)
      for (int i = j; i < soluzione.length; i++)
        estesioneSomma += insieme[i];
    
    return (somma > sommaCercata) || (somma + estesioneSomma < sommaCercata);
  }
